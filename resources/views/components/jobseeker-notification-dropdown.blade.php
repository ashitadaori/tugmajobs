...
<div class="dropdown jobseeker-notif-wrapper">
    <button type="button" class="jobseeker-notif-bell-btn {{ Auth::user()->unreadNotificationsCount > 0 ? 'has-unread' : '' }}" id="jobseekerNotifDropdown" data-bs-toggle="dropdown" aria-expanded="false">
        <i class="fas fa-bell"></i>
        @if(Auth::user()->unreadNotificationsCount > 0)
            <span class="jobseeker-notif-badge">{{ Auth::user()->unreadNotificationsCount }}</span>
        @endif
    </button>
    ...
    <div class="dropdown-menu dropdown-menu-end jobseeker-notif-dropdown" aria-labelledby="jobseekerNotifDropdown">
        ...
        <div class="jobseeker-notif-list" id="jobseekerNotifList">
            @forelse(Auth::user()->notifications()->latest()->take(5)->get() as $notification)
                ...
                <a href="{{ $redirectUrl }}"
                   class="jobseeker-notif-item {{ is_null($notification->read_at) ? 'js-notif-unread' : '' }}"
                   ...
                   data-redirect-url="{{ $redirectUrl }}">
                    ...
                    <div class="jobseeker-notif-content">
                        ...
                        <div class="jobseeker-notif-message">
                            {{ Str::limit($notification->message ?? ($data['message'] ?? 'You have a new notification'), 60) }}
                        </div>
        ...
        <div class="jobseeker-notif-footer">
            <a href="{{ route('account.notifications.index') }}" class="jobseeker-notif-view-all">
