...
@auth
    @if(auth()->user()->role === 'jobseeker')
        <button type="button" class="btn {{ $buttonClass }} btn-{{ $size }} save-job-btn {{ $attributes->get('class') }}"
            data-job-id="{{ $job->id }}" data-saved="{{ $isSaved ? 'true' : 'false' }}" aria-label="{{ $text }}">
...
@guest
    <button type="button" class="btn btn-outline-secondary btn-{{ $size }} {{ $attributes->get('class') }}"
        onclick="showLoginPrompt()" aria-label="Login to save jobs">
                    ...
                    fetch('{{ route("account.bookmarked-jobs.toggle") }}', {
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                // Update content
                                this.innerHTML = `<i class="${newIcon} me-1"></i>` + ({{ $showText ? 'true' : 'false' }} ? `<span class="save-text">${newText}</span>` : '');
