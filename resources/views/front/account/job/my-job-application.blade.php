...
<div class="applications-container">
    ...
    <header class="applications-header">
        ...
        <div class="header-inner">
            ...
            <a href="{{ route('jobs') }}" class="btn-find-jobs">
    ...
    @else
        ...
        <div class="stats-grid">
            <div class="stat-card total">
                ...
                <div class="stat-content">
                    <div class="stat-number">{{ $jobApplications->total() }}</div>
            ...
            <div class="stat-card pending">
                ...
                <div class="stat-content">
                    <div class="stat-number">{{ $jobApplications->where('status', 'pending')->count() }}</div>
            ...
            <div class="stat-card approved">
                ...
                <div class="stat-content">
                    <div class="stat-number">{{ $jobApplications->where('status', 'approved')->count() }}</div>
            ...
            <div class="stat-card rejected">
                ...
                <div class="stat-content">
                    <div class="stat-number">{{ $jobApplications->where('status', 'rejected')->count() }}</div>
        ...
        <div class="applications-list">
            @foreach ($jobApplications as $jobApplication)
                ...
                <div class="application-card {{ $jobApplication->status }}">
                    <div class="card-main">
                        <div class="job-details">
                            <a href="{{ route('jobDetail', $jobApplication->job_id) }}" class="job-title-link">
                                {{ $jobApplication->job->title }}
                            </a>
                            <div class="job-meta-tags">
                                <span class="meta-tag">
                                    <i class="fas fa-briefcase"></i>
                                    {{ $jobApplication->job->jobType->name ?? 'N/A' }}
                                </span>
                                <span class="meta-tag">
                                    <i class="fas fa-map-marker-alt"></i>
                                    {{ Str::limit($jobApplication->job->location ?? 'N/A', 30) }}
                                </span>
                                <span class="meta-tag">
                                    <i class="fas fa-users"></i>
                                    {{ $jobApplication->job->applications->count() }} applicant(s)
                                </span>
                            </div>
                        </div>
                        <div class="status-section">
                            <div class="applied-date">
                                ...
                                {{ \Carbon\Carbon::parse($jobApplication->applied_date)->format('M d, Y') }}
                            </div>
                    ...
                    @if($jobApplication->status == 'approved' && $jobApplication->statusHistory()->where('status', 'approved')->latest()->first()?->notes)
                        <div class="feedback-section success">
                            ...
                            <div class="feedback-content">
                                ...
                                <div class="feedback-text">{{ Str::limit($jobApplication->statusHistory()->where('status', 'approved')->latest()->first()->notes, 150) }}</div>
                            </div>
                        </div>
                    @endif
                        ...
                        @if($rejectionNote)
                            <div class="feedback-section danger">
                                ...
                                <div class="feedback-content">
                                    ...
                                    <div class="feedback-text">{{ Str::limit($rejectionNote, 150) }}</div>
                                </div>
                            </div>
                        @endif
                    @endif
                    ...
                    <div class="card-actions">
                        <a href="{{ route('account.showJobApplication', $jobApplication->id) }}" class="action-btn primary">
                            <i class="fas fa-eye"></i>
                            Details
                        </a>
                        <a href="{{ route('jobDetail', $jobApplication->job_id) }}" class="action-btn secondary">
                            <i class="fas fa-briefcase"></i>
                            View Job
                        </a>
                        <button type="button" class="action-btn danger" onclick="confirmRemoveApplication({{ $jobApplication->id }})">
                            <i class="fas fa-trash-alt"></i>
                            {{ $jobApplication->status == 'rejected' ? 'Remove' : 'Withdraw' }}
                        </button>
                    </div>
                </div>
                @else
                {{-- Job was deleted --}}
                <div class="application-card {{ $jobApplication->status }} job-deleted">
                    <div class="card-main">
                        <div class="job-details">
                        ...
                        <div class="status-section">
                            ...
                            <span class="status-pill {{ $jobApplication->status }}">
                                <i class="fas fa-{{ $jobApplication->status == 'pending' ? 'clock' : ($jobApplication->status == 'approved' ? 'check' : 'times') }}"></i>
...
function confirmRemoveApplication(id) {
    if (confirm("Are you sure you want to withdraw this application? You can reapply after withdrawing.")) {
        ...
        fetch('{{ route("account.removeJobs") }}', {
            method: 'POST',
            headers: {
