...
@section('jobseeker-content')
    <div class="container-fluid py-4">
        <div class="row mb-4">
            <div class="col-12">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        <h4 class="mb-1">Edit Resume</h4>
                        <p class="text-muted mb-0">Template: <span
                                class="fw-bold text-primary">{{ $resume->template->name ?? 'Default' }}</span></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('account.resume-builder.update', $resume->id) }}" method="POST" id="resumeForm"
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8">
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-header bg-white py-3">
                            <h5 class="card-title mb-0"><i class="fas fa-user-edit me-2 text-primary"></i>Personal
                                Information</h5>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="row mb-4 align-items-center">
                                <div class="col-auto">
                                    <div class="position-relative">
                                        <div id="photoPreview"
                                            style="width: 100px; height: 100px; border-radius: 50%; overflow: hidden; background-color: #f0f2f5; border: 2px solid #e1e4e8; display: flex; align-items: center; justify-content: center;">
                                            @php
                                                $photoPath = $resume->data->personal_info['photo'] ?? null;
                                            @endphp
                                            @if($photoPath && Storage::disk('public')->exists($photoPath))
                                                <img id="photoPreviewImg" src="{{ asset('storage/' . $photoPath) }}"
                                                    alt="Profile Photo" style="width: 100%; height: 100%; object-fit: cover;">
                                            @else
                                                <img id="photoPreviewImg" src="{{ asset('assets/images/avatar7.png') }}"
                                                    alt="Default Photo"
                                                    style="width: 100%; height: 100%; object-fit: cover; {{ $photoPath ? '' : 'display:none;' }}">
                                                @if(!$photoPath)
                                                    <i class="fas fa-camera text-muted" style="font-size: 32px;"
                                                        id="defaultPhotoIcon"></i>
                                                @endif
                                            @endif
                                        </div>
                                    </div>
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-header bg-white py-3">
                            <h5 class="card-title mb-0"><i class="fas fa-file-alt me-2 text-primary"></i>Professional
                                Summary</h5>
                        </div>
                        <div class="card-body">
                            <textarea name="professional_summary" class="form-control" rows="5"
                                placeholder="Write a brief overview...">{{ $resume->data->professional_summary ?? '' }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                            <h5 class="card-title mb-0"><i class="fas fa-briefcase me-2 text-primary"></i>Work Experience
                            </h5>
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                            <h5 class="card-title mb-0"><i class="fas fa-graduation-cap me-2 text-primary"></i>Education
                            </h5>
                    ...
                    <div class="card shadow-sm mb-4">
                        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                            <h5 class="card-title mb-0">Certifications <small class="text-muted ms-1">(Optional)</small>
                            </h5>
                            ...
                            <div class="card-body p-4">
                                ...
                                <div class="d-grid gap-2">
                                    <button type="button" onclick="submitForm()" class="btn btn-primary btn-lg shadow-sm">
                                        <i class="fas fa-save me-1"></i> Update Resume
                                    </button>
    ...
    @push('scripts')
        <script>
            ...
            // Ensure unique IDs for all items if they don't have one (legacy data)
            workExperiences = workExperiences.map((item, i) => ({ ...item, id: item.id || Date.now() + i }));
            educations = educations.map((item, i) => ({ ...item, id: item.id || Date.now() + i }));
            certifications = certifications.map((item, i) => ({ ...item, id: item.id || Date.now() + i }));
            projects = projects.map((item, i) => ({ ...item, id: item.id || Date.now() + i }));
