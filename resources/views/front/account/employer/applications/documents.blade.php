...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="{{ route('employer.applications.index') }}">Applications</a>
                        </li>
                        <li class="breadcrumb-item"><a
                                href="{{ route('employer.applications.show', $application->id) }}">{{ $application->user->name }}</a>
                        </li>
            ...
            <div>
                <a href="{{ route('employer.applications.show', $application->id) }}" class="btn btn-outline-primary">
                                                ...
                                                <td>
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="{{ asset('storage/' . $document['file_path']) }}"
                                                            class="btn btn-outline-primary" target="_blank" title="View Document">
                                                            <i class="bi bi-eye"></i>
                                                        </a>
                                                        <a href="{{ asset('storage/' . $document['file_path']) }}"
                                                            class="btn btn-outline-secondary" download title="Download Document">
            ...
            <div class="col-lg-4">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body text-center">
                        ...
                        @if($imgSrc)
                            <img src="{{ $imgSrc }}" alt="{{ $application->user->name }}" class="rounded-circle mb-3" width="80"
                                height="80" style="object-fit: cover;"
                                onerror="this.onerror=null; this.style.display='none'; this.nextElementSibling.style.display='inline-flex';">
                            <div class="rounded-circle mb-3 bg-light text-secondary align-items-center justify-content-center"
                                style="width: 80px; height: 80px; display: none; margin: 0 auto;">
                                <i class="bi bi-person-fill" style="font-size: 40px;"></i>
                            </div>
                        @else
                            <div class="rounded-circle mb-3 bg-light text-secondary d-inline-flex align-items-center justify-content-center"
    ...
    @push('scripts')
        <script>
            ...
            function submitStageAction(action, notes = null) {
                ...
                fetch(`/employer/applications/{{ $application->id }}/stage`, {
                    method: 'PATCH',
                    headers: {
                    ...
                    .then(data => {
                        if (data.status) {
                            alert(data.message);
                            window.location.href = '{{ route("employer.applications.show", $application->id) }}';
                        } else {
