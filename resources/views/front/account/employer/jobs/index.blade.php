...
<div class="jobs-page">
    ...
    <form method="GET" action="{{ route('employer.jobs.index') }}" class="filter-bar" id="filterForm">
        <div class="search-box">
            ...
            <input type="text" id="searchInput" name="search" placeholder="Search jobs..." value="{{ request('search') }}">
            <button type="button" class="clear-btn" id="clearSearch" style="{{ request('search') ? 'display: flex;' : 'display: none;' }}">
                <i class="bi bi-x"></i>
            </button>
        </div>
        <div class="filter-options">
            <select id="statusFilter" name="status" class="filter-select">
                ...
                <option value="{{ App\Models\Job::STATUS_APPROVED }}" {{ request('status') == App\Models\Job::STATUS_APPROVED ? 'selected' : '' }}>Active</option>
                <option value="{{ App\Models\Job::STATUS_PENDING }}" {{ request('status') == App\Models\Job::STATUS_PENDING ? 'selected' : '' }}>Pending</option>
                <option value="{{ App\Models\Job::STATUS_REJECTED }}" {{ request('status') == App\Models\Job::STATUS_REJECTED ? 'selected' : '' }}>Rejected</option>
            </select>
        </div>
    </form>
    ...
    <div class="jobs-list">
        @forelse($jobs as $job)
            ...
            @endphp
            <div class="job-card {{ $isRejected ? 'rejected' : '' }}" data-status="{{ $job->status }}">
                <div class="job-main">
                    <div class="job-info">
