...
function performSearch(query, type) {
    ...
    fetch(`{{ route('admin.search.global') }}?q=${encodeURIComponent(query)}&type=${type}`)
        .then(res => res.json())
        .then(data => {
...
function loadRecentSearches() {
    fetch('{{ route('admin.search.get-recent') }}')
        .then(res => res.json())
        .then(searches => {
...
function saveRecentSearch(query, type) {
    fetch('{{ route('admin.search.save-recent') }}', {
        method: 'POST',
        headers: {
...
function clearRecentSearches() {
    ...
    fetch('{{ route('admin.search.clear-recent') }}', {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        }
...
function loadSavedPresets() {
    fetch('{{ route('admin.search.get-presets') }}')
        .then(res => res.json())
        .then(presets => {
...
function confirmSavePreset() {
    ...
    fetch('{{ route('admin.search.save-preset') }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
function deletePreset(id) {
    ...
    fetch(`{{ route('admin.search.delete-preset', '') }}/${id}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        }
