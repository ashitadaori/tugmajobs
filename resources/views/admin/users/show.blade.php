                ...
                <div class="card-body text-center">
                    <div class="mb-3">
                        @if($user->profile_photo)
                            <img src="{{ asset('storage/' . $user->profile_photo) }}" alt="{{ $user->name }}" class="rounded-circle" style="width: 120px; height: 120px; object-fit: cover;">
                        @else
                    ...
                    <span class="badge bg-{{ $user->role === 'employer' ? 'primary' : ($user->role === 'jobseeker' ? 'success' : 'danger') }}">
                        {{ ucfirst($user->role) }}
                    </span>
                ...
                <div class="card-body">
                    <table class="table table-sm">
                        ...
                        <tr>
                            <th>KYC Status:</th>
                            <td>
                                <span class="badge bg-{{ $user->kyc_status === 'verified' ? 'success' : ($user->kyc_status === 'in_progress' ? 'warning' : 'secondary') }}">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <a href="{{ route('admin.jobs.show', $application->job_id) }}">{{ $application->job->title }}</a>
                                        </td>
            ...
            @if(($user->role === 'employer' && count($jobs) === 0) || ($user->role === 'jobseeker' && count($applications) === 0))
                <div class="card">
                    <div class="card-body text-center py-5">
                        ...
                        <p class="text-muted">This user hasn't {{ $user->role === 'employer' ? 'posted any jobs' : 'applied to any jobs' }} yet.</p>
                    </div>
                </div>
            @endif
        </div>
