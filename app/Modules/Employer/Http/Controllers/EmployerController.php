...
namespace App\Modules\Employer\Http\Controllers;
...
class EmployerController extends Controller
{
    ...
    public function updateProfile(Request $request)
    {
        try {
            ...
            return redirect()
                ->route('account.employer.profile.edit')
    ...
    public function storeJob(Request $request)
    {
        ...
        try {
            ...
            if ($request->salary_min && $request->salary_max) {
                ...
                $job->salary_range = "₱{$request->salary_min} - ₱{$request->salary_max} per month";
            }
            ...
            $job->status = $request->is_draft ? Job::STATUS_DRAFT : Job::STATUS_PENDING;
            $job->save();

            return response()->json([
                'status' => true,
                'message' => $request->is_draft ? 'Job saved as draft.' : 'Job submitted for review.',
                'redirect' => route('account.employer.dashboard')
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
    ...
    public function editJob(Job $job)
    {
        ...
        // Ensure only active categories and job types are loaded
    ...
    public function updateJob(Request $request, Job $job)
    {
        ...
        try {
            ...
            return response()->json([
                'status' => true,
                'message' => $request->is_draft ? 'Job saved as draft.' : ($job->status === Job::STATUS_PENDING ? 'Job submitted for review.' : 'Job updated successfully.'),
                'redirect' => route('account.employer.jobs.index')
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
    ...
    public function deleteJob(Job $job)
    {
        ...
        return redirect()->route('account.employer.jobs.index')
